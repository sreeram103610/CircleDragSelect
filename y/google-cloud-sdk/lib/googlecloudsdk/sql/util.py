...
class PrettyPrinter(object):
  ...
  def PrintInstance(self, instance):
    ...
    for key, value in sorted(instance.items()):
      ...
      elif key == 'replicaNames':
        self.Print('replicaNames:')
        self.Print('[')
        self.indent += 2
        self.PrintReplicaNames(value)
        self.indent -= 2
        self.Print(']')
  ...
  def PrintReplicaNames(self, replica_names):
    """Pretty prints replica names object.

    Args:
      replica_names: A list of replica names for the given instance.
    """
    for replica_name in replica_names:
      self.Print(replica_name)
...
def GetCertFromName(
  ...
  In some versions of the SQL API, the last parameter of the URL is the
  sha1fingerprint, which is not something writeable or readable by humans.
  Instead, the CLI will ask for the common name. To allow this, we first query
  all the ssl certs for the instance, and iterate through them to find the one
