...
class Create(base.Command):
  ...
  def IsReadReplica(self, args):
    """Checks if the instance being created is a read replica instance.
    ...
    Returns:
      True if the instance being created is a read replica, False otherwise.
    """
  ...
  def Run(self, args):
    ...
    # For Cloud SQL instances the defaults are as follows:
    # replication = SYNCHRONOUS
    # activation policy = ON_DEMAND
    default_replication = 'SYNCHRONOUS'
    default_activation_policy = 'ON_DEMAND'
    if self.IsReadReplica(args):
      body['masterInstanceName'] = args.master_instance_name
      # For Cloud SQL read replica instances the defaults are as follows:
      # replication = ASYNCHRONOUS
      # activation policy = ALWAYS
      default_replication = 'ASYNCHRONOUS'
      default_activation_policy = 'ALWAYS'

    if not replication:
      settings['replicationType'] = default_replication
    else:
      settings['replicationType'] = replication

    if not activation_policy:
      settings['activationPolicy'] = default_activation_policy
    else:
