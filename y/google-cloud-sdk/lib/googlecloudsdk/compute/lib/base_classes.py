...
from googlecloudsdk.core import resources as resource_parser
...
class BaseLister(BaseCommand):
  ...
  def PopulateResourceFilteringStructures(self, args):
    ...
    for name in args.names:
      try:
        ref = resource_parser.Parse(name)
        ...
        continue
      except resource_parser.UserError:
        pass
...
class GlobalRegionalDescriber(BaseDescriber):
  ...
  def CreateReference(self, args):
    try:
      ref = resource_parser.Parse(args.name, params={'region': args.region})
    except resource_parser.UnknownCollectionException:
