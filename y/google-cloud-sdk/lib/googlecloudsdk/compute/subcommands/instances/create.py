  ...
  def CreateRequests(self, args):
    ...
    if create_boot_disk:
      ...
      if (check_for_windows_image and
          image_utils.HasWindowsLicense(image_resource)):
    ...
    for instance_ref in instance_refs:
      disks, boot_disk_ref = self.CreateAttachedDiskMessages(args, instance_ref)
      if create_boot_disk:
    ...
    if check_for_windows_image and existing_boot_disks:
      ...
      for disk_resource in disk_resources:
        if image_utils.HasWindowsLicense(disk_resource):
    ...
    if add_windows_credentials_to_metadata:
      if not windows_username_present:
        username = self.context['project'].split(':')[-1][
            :constants.MAX_WINDOWS_USERNAME_LENGTH]
        metadata.items.append(messages.Metadata.ItemsValueListEntry(
