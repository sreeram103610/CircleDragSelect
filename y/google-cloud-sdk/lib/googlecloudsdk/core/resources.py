...
class Resource(object):
  ...
  def Resolve(self):
    ...
    for param in self.__ordered_params:
      ...
      if value is None:
        raise UnknownFieldException(self.__collectionpath, param)
      ResolveParam(value)
...
class Registry(object):
  ...
  def _SwitchAPI(self, api):
    ...
    old_collections = []
    for collection, parser in self.parsers_by_collection.iteritems():
      # pylint:disable=protected-access
      if parser.client._PACKAGE == api._PACKAGE:
